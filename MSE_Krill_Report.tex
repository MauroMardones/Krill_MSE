...
\begin{longtable}[]{@{}ccl@{}}
\caption{Age-specific natural mortality for Antarctic krill.}\tabularnewline
\toprule\noalign{}
...
\hypertarget{r0-scaling-a-technical-calibration}{%
\subsection{R0 Scaling: A Technical Calibration}\label{r0-scaling-a-technical-calibration}}

A critical technical step when linking SS3 to \texttt{MSEtool} is resolving the incompatibility between how each platform defines unfished recruitment. SS3 reports recruitment in absolute numbers of individuals, whereas \texttt{MSEtool} internally computes unfished spawning biomass by multiplying the recruitment parameter by the spawning potential ratio (per-recruit biomass). When krill parameters (weight-at-age in tonnes) are applied directly to SS3's large unfished recruitment value, the resulting biomass estimate becomes numerically inconsistent with the SS3 assessment.

To resolve this, we back-calculated the appropriate recruitment parameter for \texttt{MSEtool} by inverting the relationship between \(B_0\), recruitment, and per-recruit biomass. This ensures that the OM's initial unfished biomass matches the SS3 estimate of 2,029,000 tonnes and that the initial depletion level (ratio of current to unfished biomass) is consistent with the stock assessment. The detailed procedure is documented in Appendix A. This calibration is essential for producing biologically meaningful projections and ensuring that the OM correctly represents the initial stock condition.

\hypertarget{recruitment-stochasticity-and-uncertainty}{%
\subsection{Recruitment Stochasticity and Uncertainty}\label{recruitment-stochasticity-and-uncertainty}}
...
\hypertarget{appendix-a-operating-model-equations}{%
\section{Appendix A: Operating Model Equations}\label{appendix-a-operating-model-equations}}

The core equations governing the age-structured operating model for Antarctic krill are summarized below. These equations are implemented stochastically within \texttt{MSEtool} across 48 independent simulations.

\hypertarget{a0.-array-dimensions-in-msetool}{%
\subsection{A0. Array Dimensions in MSEtool}\label{a0.-array-dimensions-in-msetool}}
...
\hypertarget{a1.-population-dynamics}{%
\subsection{A1. Population Dynamics}\label{a1.-population-dynamics}}

\textbf{Age-transition equation:}
\[N_{a,t+1} = N_{a,t} \, e^{-(F_{a,t} + M_{a,t})}\]

\textbf{Plus-group (age 8 and older):}
...
\hypertarget{a2.-recruitment}{%
\subsection{A2. Recruitment}\label{a2.-recruitment}}

\textbf{Beverton-Holt stock-recruitment relationship:}
\[R_t = \frac{4 h R_0 \, \text{SSB}_t}{B_0(1-h) + \text{SSB}_t(5h-1)} \cdot \epsilon_t\]

\textbf{Recruitment deviations (AR(1) log-normal):}
...
\hypertarget{a3.-spawning-stock-biomass}{%
\subsection{A3. Spawning Stock Biomass}\label{a3.-spawning-stock-biomass}}

\[\text{SSB}_t = \sum_a N_{a,t} \, w_{a,t} \, m_{a,t}\]

where \(w_{a,t}\) is weight-at-age and \(m_{a,t}\) is maturity-at-age.

\hypertarget{a4.-growth-parameters}{%
\subsection{A4. Growth Parameters}\label{a4.-growth-parameters}}

\textbf{Von Bertalanffy length-at-age:}
...
\hypertarget{a5.-r0-calibration-for-msetool}{%
\subsection{A5. R0 Calibration for MSEtool}\label{a5.-r0-calibration-for-msetool}}

The unfished recruitment parameter for \texttt{MSEtool} is back-calculated from the SS3 biomass estimate:
...
\hypertarget{a6.-harvest-control-rules}{%
\subsection{A6. Harvest Control Rules}\label{a6.-harvest-control-rules}}

\textbf{Constant Catch (CC145):}
\[\text{TAC}_t = 145{,}000 \, \text{t}\]

\textbf{Proportional Biomass (HCR Proportional):}
...
\hypertarget{appendix-b-key-r-code}{%
\section{Appendix B: Key R Code}\label{appendix-b-key-r-code}}

\hypertarget{r0-scaling-implementation}{%
\subsection{R0 Scaling Implementation}\label{r0-scaling-implementation}}
...
\hypertarget{proportional-hcr-implementation}{%
\subsection{Proportional HCR Implementation}\label{proportional-hcr-implementation}}
