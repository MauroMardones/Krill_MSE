...
# testing for krill fixed TAC
# fixedTAC_rule <- function(stk, ctrl, args, output="catch", tracking) {
#   # Extraer año actual (ay) y año de implementación (iy)
#   ay <- args$ay
#   iy <- args$iy
#   
#   # Definir la cuota fija (ejemplo: promedio histórico de capturas)
#   fixed_TAC <- mean(catch(stk)[, ac((iy-10):(iy-1))])  # Últimos 10 años
#   
#   # Crear control para aplicar el TAC fijo
#   ctrl <- fwdControl(
#     list(year = iy, quant = "catch", value = fixed_TAC)
#   )
#   
#   # Registrar el TAC en tracking (opcional)
#   track(tracking, "fixed_TAC", ay) <- fixed_TAC
#   
...
```{r alternative, fig.height=8,fig.width=4,warning=FALSE,message=FALSE, fig.cap = paste0("Alternative HCR formulations, with $F_{min}=0$ (top) and $F_{min}=0.5F_{tgt}$ (bottom) under different $F_{adv}$ options set at $F_{adv}=F_{adv}$, $F_{adv}=0.8F_{adv}$ and at $F_{adv}=0.7F_{adv}$ from left to right")}
...
```{r eval=FALSE, fig5, fig.height=8,fig.width=10, fig.cap = paste0("MSE kobe plot Advice rules"), warning=FALSE}
## MSE kobe plot
